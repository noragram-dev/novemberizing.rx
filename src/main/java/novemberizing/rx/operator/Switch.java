...
public class Switch<T, U> extends Operator<T, U> {
    ...
    public <V> Switch<T, U> Case(int i, novemberizing.rx.func.Single<T, V> f){
        if(__default!=null){
            Log.e("", new RuntimeException("__default!=null"));
        } else {
            if (__cases.get(i) != null) {
                Log.w("", new RuntimeException("__cases.get(novemberizing.rx)!=null"));
            }
            __cases.put(i, Op(f));
        }
        return this;
    ...
    public <V> Switch<T, U> Default(novemberizing.rx.func.Single<T, V> f){
        if(__default==null){
            __default = Op(f);
        } else {
            Log.e("", new RuntimeException("__default!=null"));
