...
public class CompletionPort<T, U> extends Operator<Collection<T>, Collection<U>> {
    ...
    public static class Local<T extends Collection> extends novemberizing.rx.Local<T> {
    ...
    public CompletionPort(novemberizing.rx.func.Single<T, ?> f){
        __op = Op(f);
    }
    ...
    @Override
    public novemberizing.rx.Local<Collection<T>> declare(Task<Collection<T>> task){
